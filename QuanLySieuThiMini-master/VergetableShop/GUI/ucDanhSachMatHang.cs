...
namespace BookShop.GUI
{
    public partial class ucDanhSachMatHang : UserControl
    {
        ...
        {
            if (MessageBox.Show("Bạn muốn bỏ loại thuế này?", "Xóa", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                try
                {
                    int xx = Convert.ToInt16(dgvMATHANG.GetFocusedRowCellValue("ID").ToString());
                    int y = Convert.ToInt16(dgvThue.GetFocusedRowCellValue("ID").ToString());
                    var dbEntry = db.THUE_MATHANG.SingleOrDefault(x => x.ID_MATHANG == xx && x.ID_THUE == y);
                    db.THUE_MATHANG.Remove(dbEntry);
                    db.SaveChanges();
        ...
        void LoadThue()
        {
            dgcThue.DataSource = db.THUE_MATHANG.ToList()
                .Where(p => p.ID_MATHANG == Convert.ToInt16(dgvMATHANG.GetFocusedRowCellValue("ID").ToString()))
                .Select(p => new
                {
                    ID = p.ID_THUE,
                    Ten = db.THUEs.Where(x => x.ID == p.ID_THUE).FirstOrDefault().Ten,
                    ThueSuat = db.THUEs.Where(x => x.ID == p.ID_THUE).FirstOrDefault().ThueXuat.ToString() + "%"
                });
        }

        private void simpleButton1_Click(object sender, EventArgs e)
        {
            FrmThemThue f = new FrmThemThue(Convert.ToInt16(dgvMATHANG.GetFocusedRowCellValue("ID").ToString()));
            f.ShowDialog();
            LoadThue();
        }
