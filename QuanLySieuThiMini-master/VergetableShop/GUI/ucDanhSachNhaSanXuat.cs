...
namespace BookShop.GUI
{
    public partial class ucDanhSachNhaSanXuat : UserControl
    {
        ...
        {
            UpdateDetail();
            LoadGTri(Convert.ToInt16(dgvNhaXuatBan.GetFocusedRowCellValue("ID").ToString()));
            try
            {
                index1 = index;
        ...
        void LoadGTri(int NSX_ID)
        {
            int j = 0;
            var listNhap = (from mathang in db.MATHANGs.Where(p => p.NHASANXUATID == NSX_ID).ToList()
                            select new
                            {
                                STT = ++j,
                                MatHang = Helper.TenSanPham(mathang),
                                GiaNhap = Helper.GiaNhap(mathang),
                                SoLuong = Helper.SoLuongALL(mathang),
                                ThanhTien = (Helper.GiaNhap(mathang) * Helper.SoLuongALL(mathang)).ToString("N0"),
                                gt = Helper.GiaNhap(mathang) * Helper.SoLuongALL(mathang)
                            })
                            .ToList();
            gridControl1.DataSource = listNhap;
            label_Tong.Text = listNhap.Sum(p => p.gt).ToString("N0");
        }
        #endregion
    }
}
